    ...
    <section className={`py-16 lg:py-24 bg-white overflow-hidden ${className || ''}`}>
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          {/* Content Column */}
          <div
            className={`flex flex-col ${imagePosition === 'left' ? 'lg:order-2' : 'lg:order-1'}`}
          >
            {overline && (
            ...
            {ctaText && ctaHref && (
              <div>
                <Button
                  ...
                  href={ctaHref}
                  size="lg"
                  className="bg-brand-primary text-white hover:bg-blue-700 px-8 shadow-lg shadow-blue-900/20 group"
                >
                  {ctaText}
                  <svg
            ...
            <div className="relative">
              ...
              {overlayText && (
                <div className="absolute -bottom-6 -right-6 lg:-right-12 bg-brand-primary text-white p-8 max-w-xs lg:max-w-sm shadow-2xl z-20 hidden md:block rounded-xl border border-white/10 backdrop-blur-sm">
                  <p className="text-lg lg:text-xl font-medium leading-snug">{overlayText}</p>
                </div>
              )}
            </div>
          </div>
